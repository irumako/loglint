...
| _Какая логика у правила (`sensitive-data`)?_                                              | _Для `sensitive-data` проверяем строковые литералы по ключевым словам, а все нестроковые выражения тоже считаем подозрительными и репортим, потому что они потенциально могут содержать чувствительные данные_ |
